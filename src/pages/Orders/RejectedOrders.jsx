  ...
  return (
    <>
      <Box sx={{ p: 3 }}>
              ...
              {currentItems.length > 0 ? (
                currentItems.map((item, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell onClick={() => handleopenorder(item)}>
                      <u>{item.id}</u>
                    </TableCell>
                    <TableCell onClick={() => handleCustomerOpen(item)}>
                      <u>{item.user?.name}</u>
                    </TableCell>
                    <TableCell >{item.created_at.split("T")[0]}</TableCell>
                    <TableCell >{item.created_at.split("T")[1].split(".")[0]}</TableCell>
                    <TableCell style={{ color: "blue", cursor: "pointer" }} onClick={() => handleaddressOpen(item)}>
                      <u>View</u>
                    </TableCell>
                    <TableCell>{item.payment_option}</TableCell>
                    <TableCell >{item.payment_status}</TableCell>
                    <TableCell >
                      <a href={item.Track_id} target='_blank'>{item.Track_id}</a>
                    </TableCell>
                    <TableCell style={{ color: "blue", cursor: "pointer" }} onClick={() => handleReasonOpen(item)}>
                      <u>View</u>
                    </TableCell>
                    <TableCell >
        ...
        {/* modal for view Reasons */}
        <Modal open={viewReason} onClose={() => setViewReason(false)}>
          <Box
            sx={{
              position: 'absolute',
              top: '50%',
